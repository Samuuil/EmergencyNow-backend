...
export class UpdateProfileMedicalFields1766921312120 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."profiles_bloodtype_enum" AS ENUM('A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-', 'OTHER')`,
    );

    await queryRunner.query(`ALTER TABLE "profiles" ADD "dateOfBirth" date`);
    await queryRunner.query(
      `ALTER TABLE "profiles" ADD "bloodType" "public"."profiles_bloodtype_enum"`,
    );
    await queryRunner.query(
      `ALTER TABLE "profiles" ADD "medicines" text array`,
    );
    await queryRunner.query(
      `ALTER TABLE "profiles" ADD "illnesses" text array`,
    );
  }
